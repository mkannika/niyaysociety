...
if (isset($entry)) { 
	...
	<div class="alert alert-warning" role="alert">
		<p>เพื่อการค้นหาที่มีประสิทธิภาพ กรุณากรอกข้อมูลเกี่ยวกับเนื้อหา<strong>ทุกช่อง</strong><br>
		<strong>1. "ชื่อเรื่อง"</strong> คือ ชื่อของเรื่องนิยายหรือชื่อตอนของคุณ<br>
		<strong>2. "คำอธิบายเกี่ยวกับเรื่องนี้"</strong> คือ คำโปรยปราย หรือเนื้อหาโดยย่อ หรือเนื้อที่ชวนให้อยากอ่านของเรื่อง<br>
		<strong>3. "Meta Title"</strong> คือชื่อเรื่อง<strong>เหมือนกัน</strong>กับ "ชื่อเรื่อง" ช่องแรก<br>
		<strong>4. "Meta Keywords"</strong> เพื่อง่ายต่อการค้นหากรุณาใส่คีเวิร์ดว่าเรื่องของท่านเกี่ยวกับอะไร เช่น แม่มด, ต่อสู้, รักสามเส้า<br>
		<strong>5. "ปกนิยาย"</strong> ขนาดที่เหมาะสมคือ <strong>290x435 pixel</strong></p>
	</div>
	
	<fieldset>
	<legend><?php echo t("ข้อมูลเรื่อง")?></legend>
	<?php if ($asl->allowEditName()) { ?>
	<div class="control-group">
		<?php echo $form->label('cName', t('ชื่อเรื่อง'))?>
		<div class="controls"><?php echo $form->text('cName', Loader::helper("text")->entities($name), array('class' => 'input-xlarge', 'onKeyUp' => "ccm_updateAddPageHandle()", 'placeholder' => 'เช่น ซากุระบานที่เมืองไทย'))?></div>
	</div>
	<?php } ?>
	...
	<div class="control-group">
		...
		<div class="controls"><?php echo $form->text('cHandle', $handle, array('class' => 'span3', 'placeholder' => 'เช่น sakura-baan-tee-muang-thai'))?>
	...
	<div class="control-group">
		<?php echo $form->label('cDescription', t('คำอธิบายเกี่ยวกับเรื่องนี้'))?>
	...
	<div class="control-group" style="display:none;">
		<?php echo $form->label('cDatePublic', t('วันที่ลง'))?>
...
<?php } ?>
	...
	<fieldset>
	<legend><?php echo t('เพิ่มนิยายเรื่องนี้ไปยัง...')?></legend>
	<div class="control-group">
	...
	if ($ct->getCollectionTypeComposerPublishMethod() == 'PAGE_TYPE' || $ct->getCollectionTypeComposerPublishMethod() == 'CHOOSE') { ?>
		...
		<a href="javascript:void(0)" onclick="ccm_openComposerPublishTargetWindow(false)"><?php echo t('เลือกสถานที่ตั้ง')?></a>
	...
	<fieldset>
	<legend><?php echo t('คุณสมบัติและเนื้อหา')?></legend>
	<?php 
...
<?php } else { ?>
	...
	<?php echo Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper(t('เพิ่มเนื้อหา'), false, 'span10 offset1')?>
	
	<?php if (count($ctArray) > 0) { ?>
	<h3><?php echo t('กรุณาเลือกประเภทของเนื้อหาที่คุณต้องการเพิ่ม ?')?></h3>
